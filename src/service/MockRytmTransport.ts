...
export type MockRytmTransportMessage =
  ...
  | { type: "active_scene"; input: RytmSetActiveSceneInput }
  | { type: "performance_macro"; input: RytmSetPerformanceMacroInput }
...
export class MockRytmTransport {
  ...
  async setActiveScene(input: RytmSetActiveSceneInput): Promise<void> {
    this.requireConnected();
    this.messages.push({ type: "active_scene", input: structuredClone(input) });
  }

  async setPerformanceMacro(input: RytmSetPerformanceMacroInput): Promise<void> {
    this.requireConnected();
    this.messages.push({ type: "performance_macro", input: structuredClone(input) });
  }
