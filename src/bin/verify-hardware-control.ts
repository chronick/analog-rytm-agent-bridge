...
const operations: RytmPersistentOperation[] = [
  ...
  },
  {
    type: "set_kit_parameter",
    track: "BD",
    parameter: "retrig.velocity_curve",
    value: 5,
  },
  {
    type: "set_kit_parameter",
    track: "BD",
    parameter: "retrig.always_on",
    value: true,
  },
  {
    type: "set_sound_parameter",
  ...
  },
  {
    type: "set_global_parameter",
    section: "midi_sync",
    parameter: "clock_send",
    value: true,
  },
  {
    type: "set_global_parameter",
    section: "midi_port",
    parameter: "receive_notes",
    value: false,
  },
  {
    type: "set_global_parameter",
    section: "midi_channels",
    parameter: "auto_channel",
    value: 16,
  },
  {
    type: "set_global_parameter",
    section: "sequencer",
    parameter: "quantize_live_rec",
    value: true,
  },
  {
    type: "set_global_parameter",
...
export async function runHardwareControlVerification(): Promise<void> {
  ...
  try {
    ...
    const projected = certificationFieldsFromObjectState(
      asRecord(dryRun.projectedState),
    );
    ...
    if (!execute) {
      ...
      await client.snapshotState({
        snapshotId,
        label: "Hardware control certification baseline",
      });
      ...
      const observed = certificationFields(
        asRecord(await client.inspectDeviceState()),
      );
      ...
      const restored = certificationFields(
        asRecord(await client.inspectDeviceState()),
      );
      assert.deepEqual(restored, baseline);
      process.stderr.write(
        "snapshot rollback and baseline comparison verified\n",
      );
...
function certificationFields(
  ...
  const machineParameterBody = asRecord(
    machineParameters[Object.keys(machineParameters)[0]],
  );
  ...
  return {
    ...
    retrigVelocityCurve: asRecord(
      (kit.retrig as Array<Record<string, unknown>>)[0]?.parameters,
    ).velocity_curve,
    retrigAlwaysOn: asRecord(
      (kit.retrig as Array<Record<string, unknown>>)[0]?.parameters,
    ).always_on,
    ...
    autoChannel: asRecord(asRecord(asRecord(midi.channels)).auto_channel)
      .Channel,
...
function certificationFieldsFromObjectState(
  state: Record<string, unknown>,
): Record<string, unknown> {
